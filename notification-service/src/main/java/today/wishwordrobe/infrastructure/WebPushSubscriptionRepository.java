...
public interface WebPushSubscriptionRepository extends ReactiveMongoRepository<WebPushSubscriptionDocument,String>{
 ...
 //가장 오래된 구독
  Mono<WebPushSubscriptionDocument> findFirstByUserIdAndIsActiveOrderByLastUsedAtAsc(Long userId, boolean isActive);

  Flux<WebPushSubscriptionDocument> findByUserIdAndIsActiveOrderByLastUsedAtAsc(Long userId, boolean isActive);

  //사용자별 활성 구독 목록
  Flux<WebPushSubscriptionDocument> findByUserIdAndIsActive(Long userId,boolean isActive);
